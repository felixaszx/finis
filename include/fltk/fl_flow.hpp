...
namespace fl
{
    struct Exception
    {
        inline Exception(const char* _message)
            : m_message(_message)
        {
        }
    ...
    struct Transform
    {
        ...
        inline bool colliding(Transform& _other)
        {
            if (m_x <= _other.m_x)
            {
    ...
    struct Flow : Fl_Group
    {
        inline Flow(int _x = 0, int _y = 0, int _w = 1, int _h = 1, const char* _label = 0)
        ...
        inline void rule(Fl_Widget_Tracker _widget, const std::string& _instructions)
        {
            ...
            for (size_t ci = 0; ci < _instructions.length(); ++ci)
            {
                ...
                if (c == '=')
                {
                    type = Instruction::EXPAND;
                    continue;
                }
                else if (c == '/')
                {
                    type = Instruction::CENTER;
                    continue;
                }

                Instruction instruction;
                instruction.m_widget = _widget;
                instruction.m_instruction = Instruction::decode(c, type);
      ...
      protected:
        inline void process()
        {
            ...
            {
                Instruction& i = m_instructions.at(ii);

                if (i.m_instruction == Instruction::MOVE_LEFT || i.m_instruction == Instruction::MOVE_RIGHT ||
                    i.m_instruction == Instruction::MOVE_UP || i.m_instruction == Instruction::MOVE_DOWN ||
                    i.m_instruction == Instruction::EXPAND_LEFT || i.m_instruction == Instruction::EXPAND_RIGHT ||
                    i.m_instruction == Instruction::EXPAND_UP || i.m_instruction == Instruction::EXPAND_DOWN ||
                    i.m_instruction == Instruction::CENTER_LEFT || i.m_instruction == Instruction::CENTER_RIGHT ||
                    i.m_instruction == Instruction::CENTER_UP || i.m_instruction == Instruction::CENTER_DOWN)
                {
                    int xDir = i.x_direction();
                    int yDir = i.y_direction();

                    Transform wt(i.m_widget, m_padding);
                    ...
                    while (true)
                    {
                        ...
                        else if (i.m_instruction == Instruction::EXPAND_LEFT ||
                                 i.m_instruction == Instruction::EXPAND_RIGHT ||
                                 i.m_instruction == Instruction::EXPAND_UP ||
                                 i.m_instruction == Instruction::EXPAND_DOWN ||
                                 i.m_instruction == Instruction::CENTER_LEFT ||
                                 i.m_instruction == Instruction::CENTER_RIGHT ||
                                 i.m_instruction == Instruction::CENTER_UP ||
                                 i.m_instruction == Instruction::CENTER_DOWN)
                        {
                            wt.scale(xDir, yDir);
                        }
                        else
                        ...
                        {
                            State& s = m_states.at(si);
                            if (!s.m_placed)
                                continue;
                            if (s.m_widget.widget() == i.m_widget.widget())
                                continue;

                            Transform st(s.m_widget, 0);
                    ...
                    if (i.m_instruction == Instruction::CENTER_LEFT || i.m_instruction == Instruction::CENTER_RIGHT ||
                        i.m_instruction == Instruction::CENTER_UP || i.m_instruction == Instruction::CENTER_DOWN)
                    {
        ...
        inline void prepare()
        {
            ...
            for (size_t si = 0; si < m_states.size(); ++si)
            {
                ...
                Helper::position(t, w() - t.widget()->w() - m_padding, h() - t.widget()->h() - m_padding);
            }
        }
    };
}; // namespace fl
