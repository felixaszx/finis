...
IMPL_OBJ_NEW(vk_shader, vk_ctx* ctx, const char* file_path, VkShaderStageFlags stage)
{
    ...
    this->desc_sets_ = alloc(vk_shader_desc_set, 10);
    this->set_limit_ = 10;
    for (size_t i = 0; i < this->set_size_; i++)
    {
        this->desc_sets_[i].binding_limit_ = 10;
        this->desc_sets_[i].desc_name_ = calloc(this->desc_sets_[i].binding_size_, //
                                                sizeof(*this->desc_sets_[i].desc_name_));
        this->desc_sets_[i].bindings_ = alloc(VkDescriptorSetLayoutBinding, //
                                              this->desc_sets_[i].binding_size_);
    }
    ...
    this->push_const_.stageFlags = stage;
...
{
    for (size_t i = 0; i < this->set_limit_; i++)
    {
        ffree(this->desc_sets_[i].desc_name_);
        ffree(this->desc_sets_[i].bindings_);
    }
    ffree(this->desc_sets_);
...
IMPL_OBJ_NEW_DEFAULT(vk_gfx_pl)
{
    ...
    this->multi_sample_.sType = VK_STRUCTURE_TYPE_PIPELINE_MULTISAMPLE_STATE_CREATE_INFO;
    ...
    this->cinfo_.pMultisampleState = &this->multi_sample_;
...
IMPL_OBJ_DELETE_DEFAULT(vk_gfx_pl)

IMPL_OBJ_NEW_DEFAULT(vk_comp_pl)
{
    this->cinfo_.sType = VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO;
    this->cinfo_.stage = this->stage_;
    return this;
}

IMPL_OBJ_DELETE_DEFAULT(vk_comp_pl)