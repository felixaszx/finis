                                                     ...
                                                     size_t gltf_idx)
{
    ...
    for (const fgltf::Animation& anim_iter : gltf.animations)
    {
        futs.emplace_back(th_pool.submit_task(
            [&gltf, anim_idx, &animations, &res_structure, &res_details, &gltf_idx]()
            {
                ResAnimation& anim = animations[anim_idx];
                const fgltf::Animation& anim_in = gltf.animations[anim_idx];
                    ...
                    switch (channel.path)
                    {
                        case fgltf::AnimationPath::Translation:
                        {
                            ...
                            fgltf::iterateAccessor<float>( //
                                gltf, timeline_acc,        //
                                [&](float time_point) { target->translation_timeline_.push_back(time_point * 1000); });
                        ...
                        case fgltf::AnimationPath::Rotation:
                        {
                            ...
                            fgltf::iterateAccessor<float>( //
                                gltf, timeline_acc,        //
                                [&](float time_point) { target->rotation_timeline_.push_back(time_point * 1000); });
                        ...
                        case fgltf::AnimationPath::Scale:
                        {
                            ...
                            fgltf::iterateAccessor<float>( //
                                gltf, timeline_acc,        //
                                [&](float time_point) { target->scale_timeline_.push_back(time_point * 1000); });
                            fgltf::iterateAccessor<fgltf::math::fvec3>(
                                gltf, sample_acc, //
                                [&](const fgltf::math::fvec3& sample)
                        ...
                        case fgltf::AnimationPath::Weights:
                        {
                            ...
                            fgltf::iterateAccessor<float>( //
                                gltf, timeline_acc,        //
                                [&](float time_point) { target->weight_timeline_.push_back(time_point * 1000); });
                            fgltf::iterateAccessor<float>( //
                                gltf, sample_acc,          //
...
void fi::CombinedAnimChannel::sample_animation(CpuClock::MilliSecond time_point, bool stepping)
{
    ...
    {
        CpuClock::MilliSecond translation_time_point = time_point //
                                                       % (translation_timeline_.back() - translation_timeline_.front());
        translation_time_point += translation_timeline_.front();
    ...
    {
        CpuClock::MilliSecond rotation_time_point = time_point //
                                                    % (rotation_timeline_.back() - rotation_timeline_.front());
        rotation_time_point += rotation_timeline_.front();
    ...
    {
        CpuClock::MilliSecond scale_time_point = time_point //
                                                 % (scale_timeline_.back() - scale_timeline_.front());
        scale_time_point += scale_timeline_.front();
    ...
    {
        CpuClock::MilliSecond weight_time_point = time_point //
                                                  % (weight_timeline_.back() - weight_timeline_.front());
        weight_time_point += weight_timeline_.front();
