...
fi::res::gltf_file::gltf_file(const std::filesystem::path& path)
{
    ...
    std::future<void> tex_fut = th_pool.submit_task(
        [&]()
        {
            ...
            for (const auto& tex : asset_->textures)
            {
                ...
                stbi_uc* pixels = stbi_load_from_memory(
                    castf(const stbi_uc*, std::get<3>(buffer.data).bytes.data() + view.byteOffset),
                    view.byteLength, //
                    &g_tex.x_, &g_tex.y_, &g_tex.comp_, STBI_rgb_alpha);
                g_tex.comp_ = 4;
                if (pixels)
                {
                    g_tex.data_.resize(view.byteLength);
    ...
    std::future<void> mat_fut = th_pool.submit_task(
        [&]()
        {
            ...
            for (const auto& mat : asset_->materials)
            {
                ...
                g_mat.metallic_roughness_ =
                    pbr.metallicRoughnessTexture ? pbr.metallicRoughnessTexture->textureIndex : -1;
    ...
    std::vector<std::future<void>> mesh_futs;
    mesh_futs.reserve(asset_->meshes.size());
    meshes_.resize(asset_->meshes.size());

    size_t m = 0;
    for (const auto& mesh : asset_->meshes)
    {
        mesh_futs.emplace_back(th_pool.submit_task(
            [&, mesh, m]()
            {
    ...
    std::for_each(mesh_futs.begin(), mesh_futs.end(), [](std::future<void>& fut) { fut.wait(); });
    tex_fut.wait();
    mat_fut.wait();
}